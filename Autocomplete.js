...
class Autocomplete 
{
    constructor(o)
    {
        if(typeof o.element === 'undefined') {
            let msg = 'Autocomplete foi instanciado incorretamente';
            alert(msg);
            throw new Error(msg);
        }
    ...
    {        
        ...
        //this.addOuvinteClickInput();
        //this.addOuvinteBlurInput();
    }
    ...
    construcao()
    {
        //this._input.insertAdjacentHTML('beforebegin', `<select type="hidden" name="${this._nomeInput}" value=""></select>`);        
        //this._input.name = `${this._nomeInput}_autocomplete`;
    ...
    iniciaAutocomplete()
    {
        ...
        for(let i = 0; i < this._oLista.length; i++){            
            let stringDigitada = Utils.removerAcentos(texto).toUpperCase();
            let stringDaLista  = Utils.removerAcentos(this._oLista[i].name).toUpperCase();
